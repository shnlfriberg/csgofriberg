...
export default function AdminReports() {
  ...
  const columns: Column<MatchReport>[] = [
    ...
    { key: 'status', title: t('admin.reportStatusLabel'), render: (report) => <Badge text={statusLabel(report.status)} color={report.status === 'pending' ? 'amber' : report.status === 'resolved' ? 'green' : 'gray'} /> },
    { key: 'actions', title: t('admin.actions'), render: (report) => <button className="btn btn-ghost btn-sm" type="button" onClick={() => setSelected(report)}><Eye size={15} />{t('admin.handleReport')}</button> },
  ];
  ...
  return <>
    ...
    {selected && <ReportDialog report={selected} onClose={() => setSelected(null)} onSaved={(updated) => { setReports((current) => current.map((item) => item.id === updated.id ? updated : item)); setSelected(null); }} />}
  </>;
}
...
function ReportDialog({ report, onClose, onSaved }: { report: MatchReport; onClose: () => void; onSaved: (report: MatchReport) => void }) {
  ...
  const save = async () => {
    setSaving(true);
    try {
      await api.patch(`/admin/reports/${report.id}`, { status, adminNote: note.trim() });
      onSaved({ ...report, status, adminNote: note.trim(), handledAt: status === 'pending' ? null : new Date().toISOString() });
  ...
  return <ModalPortal><div className="admin-player-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div ref={dialogRef} className="admin-player-dialog admin-report-dialog" role="dialog" aria-modal="true" aria-labelledby="admin-report-title" tabIndex={-1}>
    ...
    <div className="admin-report-detail">
      ...
      <div className="confirm-actions"><button className="btn btn-ghost" type="button" onClick={onClose}>{t('common.cancel')}</button><button className="btn" type="button" disabled={saving} onClick={() => void save()}>{saving ? t('common.loading') : t('admin.saveReport')}</button></div>
    </div>
  </div></div></ModalPortal>;
}
